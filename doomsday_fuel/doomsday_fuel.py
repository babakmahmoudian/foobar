...
def solution_1(m):
    def invert(matrix):
        ...
        def get_deternminant(matrix):
            ...
            for c in range(len(matrix)):
                determinant += Fraction((-1)**c) * matrix[0][c] * \
                    get_deternminant(get_minor(matrix, 0, c))
            return determinant
    ...
    for i in non_absorbing_states:
        sum_of_row = sum(m[i])
        row = [Fraction(m[i][j - len(absorbing_states)], sum_of_row)
    ...
    std_form_f = invert(tmp_std_form_f)

    f_cross_r = multiply(std_form_f, std_form_r)

    preliminary_result = f_cross_r[0]
    lcm = lcm(preliminary_result)
